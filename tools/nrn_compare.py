...
def check_summary(summary1, summary2, dataset_filter=None, verbosity=0):
    ...
    if diff1:
        logging.error("File {} datasets not in {}: {}".format(summary1, summary2, diff1))
    diff2 = f2_keyset - f1_keyset
    if diff2:
        logging.error("File {} datasets not in {}: {}".format(summary2, summary1, diff2))
    ...
    if wrong_records:
        print("Found {} mismatching records out of {}. ({:.1f}%)"
              .format(wrong_records, total_records, float(wrong_records) * 100 / total_records))
